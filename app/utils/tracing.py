...
# Add this lightweight utility for app-level trace logging
import pprint

def trace_log(label: str, payload: Any) -> None:
    """
    Basic structured log to help trace agent and data flow logic.
    """
    log_output = f"\n📍 TRACE: {label}\n{pprint.pformat(payload, indent=2)}"
    logging.getLogger("app.tracing").info(log_output)